document.addEventListener('DOMContentLoaded', function() {
    ...
    function processWordWithCombinedChars(word) {
        // First, handle special letter substitutions
        let processedWord = word
            .replace(/[vw]/g, 'b')  // Replace v and w with b
            .replace(/x/g, 'ks')    // Replace x with ks
            .replace(/q/g, 'k');    // Replace q with k

        // Then, check for combined letter sounds in the entire word
            ...
            for (let i = 0; i < syllable.length; i++) {
                ...
                if ('aeıioöuü'.includes(char)) {
                    ...
                    if (isFirstSyllableAE) {
                        if (!firstAEFound) {
                            firstAEFound = true;
                            lastVowel = char; // Store the vowel even if it's dropped
                        }
                        continue;
                    }
                    
                    // Rule 2: If this is the last syllable and ends with a vowel, always write it
                    const isLastSyllable = s === syllables.length - 1;
                    const isLastCharacter = i === syllable.length - 1;
                    if (isLastSyllable && isLastCharacter) {
                        lastVowel = char;
                        syllableResult += turkishToTurkic[char];
                        continue;
                    }
                    
                    // Rule 3: Apply vowel dropping rule for repetition
                    if (lastVowel && (
