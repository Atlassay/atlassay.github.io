document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to translate a single word with vowel dropping rule and consonant harmony
    function translateWordWithVowelDropping(word) {
        ...
        for (let s = 0; s < syllables.length; s++) {
            ...
            for (let i = 0; i < syllable.length; i++) {
                if ('aeıioöuü'.includes(syllable[i])) {
                    syllableVowel = syllable[i];
                    break;
                }
            }
            
            // If no vowel was found in this syllable (shouldn't happen with our algorithm),
            // use the last vowel for consonant harmony
            if (!syllableVowel && lastVowel) {
                syllableVowel = lastVowel;
            }
            ...
            for (let i = 0; i < syllable.length; i++) {
                ...
                if ('aeıioöuü'.includes(char)) {
                    ...
                    if (isFirstSyllableAE) {
                        // Skip this vowel but mark it as found
                        if (!firstAEFound) {
                            firstAEFound = true;
                        }
                        continue;
                    ...
                    if (i < syllable.length - 2) {
                        const threeChars = syllable.substring(i, i + 3);
                        if (turkishToTurkic[threeChars]) {
                            let turkicChar = turkishToTurkic[threeChars];
                            
                            // Apply consonant shift for combined characters
                            if (syllableVowel && combinedCharacterPairs[turkicChar]) {
                                // If this is a combined character that has a front/back pair
                                if (frontVowels.includes(syllableVowel)) {
                                    // For front vowels (e,i,ö,ü) use the front version
                                    // For ok/uk -> ök/ük
                                    if (turkicChar === '𐰸') {
                                        turkicChar = '𐰰';
                                    }
                                } else if (backVowels.includes(syllableVowel)) {
                                    // For back vowels (a,ı,o,u) use the back version
                                    // For ök/ük -> ok/uk
                                    if (turkicChar === '𐰰') {
                                        turkicChar = '𐰸';
                                    }
                    ...
                    // Check two-character combinations if no three-char combo was found
                    if (!combinedFound && i < syllable.length - 1) {
                        const twoChars = syllable.substring(i, i + 2);
                        if (turkishToTurkic[twoChars]) {
                            let turkicChar = turkishToTurkic[twoChars];
                    ...
                    // Process single character if no combinations were found
                    if (!combinedFound) {
                        if (turkishToTurkic[char]) {
                            ...
                            if (syllableVowel && consonantPairs[turkicChar]) {
                                if (frontVowels.includes(syllableVowel)) {
                                    // For front vowels (e,i,ö,ü) use thin/palatal consonants
                                    if (backConsonants.includes(turkicChar)) {
                                        turkicChar = consonantPairs[turkicChar];
                                    }
                                } else if (backVowels.includes(syllableVowel)) {
                                    // For back vowels (a,ı,o,u) use thick/velar consonants
    ...
    turkishInput.addEventListener('input', function() {
        ...
        if (inputText === 'test') {
            ...
            console.log('Syllables:', syllables); // Should show ['gü', 'le', 'nay']
            const translated = translateWordWithVowelDropping(testWord.toLowerCase());
            console.log('Translated result:', translated);
            // Input other test cases
            const testCases = ['atatürk', 'istanbul', 'türkiye', 'ankara', 'izmir', 'turkish'];
            testCases.forEach(word => {
                console.log(`\nTesting "${word}":`);
                const sylls = divideTurkishWordIntoSyllables(word);
                console.log('Syllables:', sylls);
                const trans = translateWordWithVowelDropping(word);
                console.log('Translated result:', trans);
            });
        }
